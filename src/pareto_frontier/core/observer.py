...
class ParetoObserver:
    ...
    def get_summary(self):
        ...
        for entry in self._read_logs():
            ...
            lat = entry.get('total_latency_ms', 0) / 1000.0 if isinstance(entry.get('total_latency_ms'), (int, float)) else 0
            ...
            if score > 0: scores.append(float(score))
            count += 1
        return {
            "runs": count,
    ...
    def plot_trend(self, limit=15):
        ...
        print("{:<5} | {:<8} | {:<7} | {}".format("Run", "Cost ($)", "Acc %", "Efficiency"))
...
if __name__ == "__main__":
    ...
    try:
        ...
        if args.summary or not args.trend:
            s = obs.get_summary(); print("\n[TRACE] Metrics:", s)
        if args.trend or not args.summary: obs.plot_trend()
