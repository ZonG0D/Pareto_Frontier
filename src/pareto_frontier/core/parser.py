...
class InputParser:
    def __init__(self, endpoint: str, model: str, timeout: int = 30):
        ...
        if not endpoint.endswith("/api/chat") and not endpoint.endswith(
            "/api/embeddings"
        ):
    ...
    def _apply_quick_rules(self, text: str) -> Optional[str]:
        ...
        if (
            ...
            and any(k in text_lower for k in ["physics", "science", "nature"])
        ):
    ...
    def parse_via_ollama(self, raw_input: str) -> Optional[Dict]:
        ...
        try:
            ...
            print(
                f"[ERROR] Parsing failed, using fallback. Error: {e}", file=sys.stderr
            )
